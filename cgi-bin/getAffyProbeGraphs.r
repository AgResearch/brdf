	...
	trellis.device(
        ...
        if (includemm == "True") {
           ...
           if(yscale == "linear") {
              ...
              plotFrame=data.frame(Slide=rep(stacked.data.pm$Slide,2), probe.number=rep(stacked.data.pm$probe.number,2)  , all.intensity=c(log(stacked.data.pm$pm.intensity),log(stacked.data.pm$mm.intensity)))
           }
           colour2 = "red"
           ylabel =  c("Probe intensity (B=pm R=mm) ",yscale)
        }
        else {
           if (yscale == "linear") {
              ...
              plotFrame=data.frame(Slide=stacked.data.pm$Slide, probe.number=stacked.data.pm$probe.number , all.intensity=log(stacked.data.pm$pm.intensity))
           }
           ylabel =  c("Probe intensity ",yscale)
        }
