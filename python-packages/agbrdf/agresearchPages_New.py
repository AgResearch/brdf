...
from dataImportModule import dataSourceOb, importProcedureOb, importFunction
...
def errorPage(message):
    print htmlModule.pageWrap("Error",message)
...
class searchResultPage ( page ):
    ...
    def doSearch(self):
        ...
        for obTypeName in self.searchTypes.keys():
            ...
            if self.argDict['obTypeName'] == 'All Tables' or self.argDict['obTypeName'] == obTypeName or \
                ...
                engineQuery="select getSearchResultList(%(queryString)s,'SYSTEM',1000,%(typeName)s,0)"
    ...
    def asHTML(self):
        ...
        if self.pageState['ERROR'] == 0:
            ...
            content = '<table border=yes width=800>\n'
            ...
            return  htmlModule.pageWrap("Result of search of : " + self.argDict['obTypeName'] + " for <b>" + self.argDict['queryString'] + "</b>",content)
        else:
            return ""
...
class fetchPage ( page ):
    ...
    def asHTML(self):
        ...
        if self.argDict['context'] ==  'briefsearchsummarypage' and isinstance(self.myObject,obList):
            ...
            return htmlModule.pageWrap(" ",content)
        else:
            #return  htmlModule.pageWrap(" ",self.myObject.asHTMLTableRows(context=self.argDict['context']))
            return htmlModule.pageWrap(" ",self.myObject.asHTMLTableRows(context=self.argDict['context']), menuJS=self.myObject.getMenuBarJS(metadata=self.myObject.obState['METADATA'],context=self.argDict['context']))
        ...
        if self.argDict['page'] == 'report_microarrayextract1':
            ...
            try:
           	connection=databaseModule.getConnection()            	
                ...
                sql = """
                select obid,listdefinition,currentmembership from oblist where listtype = 'USER_PROJECT_LIST' and
                currentmembership > 0 order by createddate desc
                """
                listCursor.execute(sql)
                listTuples = listCursor.fetchall()
                        ...
                        if reportFieldDict['seqstring'] != None:
                            if len(reportFieldDict['seqstring']) > 0:
                                ...
                                if self.argDict['outputformat'] == 'html':
                                    ...
                                    if self.argDict['outputto'] == 'browser':
                                        print ">%s"%reportFieldValues['idline']
                                        (outline,currentpos) = tidyout(reportFieldValues['seqstring'], 60, 0, lineend)
                                    else:
                                        print ">%s"%reportFieldValues['idline']
                                        (outline,currentpos) = tidyout(reportFieldValues['seqstring'], 60, 0, lineend,False)
